...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if ($errors['name']) {
    ...
    $messages[] = '<div class="pas error">Заполните имя или у него неверный формат</div>';
  }
  if ($errors['email']) {
    setcookie('email_error', '', 100000);
    $messages[] = '<div class="pas error">Заполните email или у него неверный формат</div>';
  }
  if ($errors['year']) {
  ...
  if ($errors['super']) {
    ...
    $messages[] = '<div class="pas error">Выберите суперспособности.</div>';
  }
  if ($errors['bio']) {
    setcookie('bio_error', '', 100000);
    $messages[] = '<div class="pas error">Заполните биографию или у неё неверный формат</div>';
  }
  if ($errors['check-1']) {
